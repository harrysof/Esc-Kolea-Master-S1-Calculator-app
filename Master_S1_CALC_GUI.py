...
def calculate_semester_average():
    ...
    for subject in subjects:
        ...
        try:
            exam_grade = float(st.session_state.get(exam_key, 0) or 0) if st.session_state.get(exam_key) != "" else 0 # Handles empty strings and missing keys, also None
            td_grade = float(st.session_state.get(td_key, 0) or 0) if st.session_state.get(td_key) != "" else 0 # Handles empty strings and missing keys, also None
